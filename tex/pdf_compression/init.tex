...
\begin{chapabstract}
  ...
  %
  %
  % We propose a method for compressing probability distributions, which is applied to the entropy bottleneck of learned image compression models.
  % The proposed method is capable of adapting to the input distribution, rather than using a fixed dataset-optimized probability distribution which suffers from the amortization gap.
  % We demonstrate that our input-adaptive method can be used by models containing the entropy bottleneck in order to achieve better rate-distortion performance than using a single, dataset-optimized probability distribution.
  % The proposed method is simple, and can be easily integrated into existing entropy bottleneck architectures.
  % All experiments are performed using the Kodak test dataset~\cite{kodak_dataset}.
  % Our method achieves a BD-rate of $-\%$ compared to the base fully-factorized entropy bottleneck~\cite{balle2018variational}, and a BD-rate of $-...\%$ with additional RDOQ.
\end{chapabstract}
...
% TODO Slightly repetitive, wordy, and repetitive.
% TODO Explain compression theory, Shannon, optimal RD theory?
...
% Just like "side-information" in the "hyperprior" models.
...
% For example, the methods introduced by Ballé~\emph{et~al.}~\cite{balle2018variational} model a latent representation $\boldvar{y}$ using:
...
\begin{enumerate}
  \item
    % A learned non-parametric per-channel distribution, known as the "fully factorized" \emph{entropy bottleneck}.
    A "fully factorized" \emph{entropy bottleneck}. % , which consists of a learned non-parametric distribution for each channel.
...
% Interestingly, both high rate and low rate tend to be Laplacian-shaped.
...
% Are optimized for a dataset of images.
% We show that by using image-specific probability distributions, we can achieve better rate-distortion performance than using a single, dataset-optimized probability distribution.
...
% Interdigital -- "Entropy Coding Improvement for Low-complexity Compressive Auto-encoders"
% ~\cite{galpin2023entropy}
% Low-complexity non-deep raster scan context modeling, channel ordering, RDOQ applied in raster-scan order tensor per-element.
% ...
% (show images/tables for comparison?)
%
% Disney - "Content Adaptive Optimization for Neural Image Compression"
% ~\cite{campos2019content}
% https://arxiv.org/abs/1906.01223
%
% Interdigital - "Reducing The Amortization Gap of Entropy Bottleneck In End-to-End Image Compression"
% \cite{balcilar2022amortizationgap}
% https://arxiv.org/abs/2209.00964
% Transmits manually encoded quantized to 8-bit parameters that follow mixed Gaussian model
%
...
is not precisely aware of the exact effect of quantization.  % TODO reword?
...
The approximation model is then used as the encoding distribution % for the latent channel,
...
% To close the amortization gap, we propose a method that adapts to the input distribution.
% Is that why it's called FillGap?
% And compresses the distribution, of course.
% In order to understand the importance of this, ...

% One drawback is that need to measure distribution explicitly, or "estimate" a good proxy...

% TODO clarify, collection of probability distributions,
...
% $\boldvar{\hat{y}}_j = \operatorname{Quantize}[\boldvar{y}_j]$.
% The _j can be inside or outside the Q operator since every channel is quantized "independently" and "identically", i.e.,
% Q(y) = [Q(y_1), Q(y_2), ..., Q(y_M)]
% From a category theory perpective, Q(I(y)) = I(Q(y)) is a commutative diagram, where I(y) = y_j.
% Or a product category, or something like that.
% And something related I saw in group theory once... I forget.
%
% But wait... under RDOQ, the quantization is not "i.i.d.", so (Q(y))_j might be more generally accurate.
%
% Avoid cyclic definition... but I guess this is not a definition; only a property:
% The true distribution is also the distribution which minimizes the rate cost of the entropy model for encoding a specific latent representation $\boldvar{\hat{y}}$, i.e.,
...
\begin{equation*}
  \boldvar{\hat{p}}^*_j = \argmin_{\boldvar{\hat{p}}_j} \left[
    ...
    % \sum_i -p_{ji} \log \hat{p}_{ji}
  \right]
  \implies \boldvar{\hat{p}}^*_j = \boldvar{p}_j.
\end{equation*}
% where $p_{ji}$ is the probability mass represented by the $i$-th bin of the distribution $\boldvar{p}_j$.
...
Instead, we propose a method that generates a reconstructed approximation $\boldvar{\hat{p}}$ that targets $\boldvar{p}$, subject to a rate trade-off. % constraint.
% measured in KL divergence, w.r.t. lambda_q trade-off...
% D(p || phat) is the "distortion" of the distribution, measured in KL divergence.
% H(q) is the rate cost. (Slightly imprecise notation.)
% TODO Consider moving some of the "Loss function" section here. On the other hand, we haven't really introduced the architecture overview, and "q" yet... so maybe not.
...
\begin{enumerate}[label=(\roman*), noitemsep, topsep=0pt]
  ...
  \item the $y$-coordinate is the bin index $i$ of the discretized probability distribution, and
    % $\boldvar{p}_j$ or $\boldvar{\hat{p}}_j$, and % for the $j$-th channel, and
  ...
  % \item the z-coordinate (i.e., color intensity) is the negative log-likelihood (in bits) of the symbol $\hat{y}$ for that channel, i.e., $-\log \boldvar{p}_{\boldvar{\hat{y}}}(\boldvar{\hat{y}})$.
\end{enumerate}
% We call this representation the "PDF signature".

% TODO Discuss amortization further:
% Its amortized probability distribution is
% In the first row is a "default" dataset-optimized probability distribution, which is used by the standard "amortized" entropy bottleneck.


% TODO(figure): Rename "default" to "amortized"?
\begin{figure}[htbp]
  \centering
...
% TODO s/default/amortized?
% TODO A bit redundantly phrased...
...
% This is particularly true for regions of the true distribution where the probability mass is largest, which is also where accurate reconstruction is most important.
% $\boldvar{p}$ and $\boldvar{\hat{p}}$ are quite close for high-probability symbols.
...
% Or rate-distortion, where distortion is actually KL div, which is also a rate.
...
% In the entropy bottleneck, the probability distribution for a single channel is modelled by a trained cumulative logits function,
...
% First, we devise a module for acquiring the target or "true" probability distribution which
We do so by estimating the input distribution using a histogram.
% However, a direct histogram estimation method is non-differentiable, so we
...
% In order to ensure differentiability backwards through the histogram module, we use a method reminiscent of kernel density estimation (KDE).
% the method described below.

% In contrast, our histogram method places a kernel function centered at each bin, which is applied to all observations.
...
% Technically, the horizontally flipped kernel function... but kernel functions are often symmetric.
...
% We choose the kernel function $K(u)$ to be a triangular function:
\begin{equation*}
  K_{\mathrm{soft}}(u) = \max \{ 0, 1 - |u| \}.
\end{equation*}
% The functions used to estimate each bin of the histogram are visualized in \cref{fig:pdf/kernels}.
...
% approximate the gradient of the hard histogram with the gradient of the soft histogram.

% In the method of kernel density estimation (KDE), which is used to estimate the probability density function from a set of observations, a kernel function is placed centered at each observed sample, and those functions are summed to create the overall density function.
% In contrast, our histogram method places a kernel function centered at each bin, which is applied to all observations.
% However, the two methods produce equivalent results for symmetric kernel functions.
...
% In compression problems, we seek to minimize the rate of the compressed bitstreams, while also minimizing the distortion between the input and reconstructed data.
...
% Briefly, $\boldvar{x}$ is the input image, $\boldvar{y}$ is the latent representation, and $\boldvar{\tilde{y}}$ is the quantized latent representation.
...
% entropy bottleneck formulation:
% "fully factorized" cross-entropy:
\begin{equation*}
  R_{\boldvar{\tilde{q}}}  % (\boldvar{x})
  =
  \sum_{j=1}^{M}
  % \sum_{i=1}^{\operatorname{length}(\boldvar{\tilde{q}}_j)}
  \sum_{i=1}^{\operatorname{length}(\boldvar{l}_j)}
  % NOTE: Technically, p_q_j takes a vector as input.
  % -p_{\boldvar{\tilde{q}}_j}(\tilde{q}_{ji})
  % \log p_{\boldvar{\tilde{q}}_j}(\tilde{q}_{ji}),
  % NOTE: The following is "correct", but very messy:
  % -(p_{\boldvar{\tilde{q}}_j}(\boldvar{\tilde{q}}_j))_i
  % \log (p_{\boldvar{\tilde{q}}_j}(\boldvar{\tilde{q}}_j))_i,
  % NOTE Less messy:
  -{l_{ji}} \log {l_{ji}},
...
% OLD TEXT
%
% We define the total rate cost of the compressed latent representation $\boldvar{\hat{q}}$ as $R_{\boldvar{\hat{q}}} = \E [-\log p(\boldvar{\hat{q}})]$.
% The total rate cost for $\boldvar{\hat{y}}$ is defined as the cross-entropy between the true and reconstructed distributions,
% $R_{\boldvar{\hat{p}}} = - \lambda_p \sum_j \boldvar{p}_j \cdot \log \boldvar{\hat{p}}_j$, where $\lambda_p$ is a trade-off parameter between ...
% % lambda_p?
% Then, the overall rate-distortion loss function is:
% \begin{equation*}
%   \mathcal{L}
%   = R_{\boldvar{\hat{q}}}
%   % + \lambda_p \cdot -\sum_{i,j} p_{ji} \log \hat{p}_{ji}
%   + \lambda_p \sum_j -\boldvar{p}_j \cdot \log \boldvar{\hat{p}}_j
%   + \lambda_x D(\boldvar{x}, \boldvar{\hat{x}}),
% \end{equation*}
% where $\lambda_x$ is a trade-off parameter between rate and distortion.
%
...
\begin{equation}
  ...
  \begin{split}
    ...
    %   \frac{p_i}{p_i} \frac{\partial p_i}{\partial y_k}
    %   + \frac{\partial p_i}{\partial y_k} \log \hat{p}_i
    % \right] \quad \text{assuming } \hat{p}_i \approx p_i \\
    % &= \frac{-H W}{s^2} \sum_i
    % \left[ 1 + \log \hat{p}_i \right]
    % \frac{\partial p_i}{\partial y_k} \\
    % &= \frac{-1}{\Delta b} \sum_i
    %   \left[ 1 + \log \hat{p}_i \right] \cdot
...
% , i.e., the left and right bins.
% Furthermore, the gradient is non-zero only for the two bins that contain the value $y_k$.
...
\begin{equation*}
  ...
  -
  \left[
    % (y_k - b_{\lfloor{y_k - y_{\mathrm{min}}}\rfloor + 1}) \cdot
    \alpha \cdot
    \log \hat{p}_{\lfloor{y_k - y_{\mathrm{min}}}\rfloor + 1} +
    % (b_{\lceil{y_k - y_{\mathrm{min}}}\rceil + 1} - y_k) \cdot
    (1 - \alpha) \cdot
    \log \hat{p}_{\lceil{y_k - y_{\mathrm{min}}}\rceil + 1}
  \right],
\end{equation*}
...
\begin{equation*}
  ...
  % = \int_{- \frac{1}{2}}^{+ \frac{1}{2}} f(y + t) \, dt,
\end{equation*}
%
...
% TODO criticize locality, or how if p is not p_hat, then gradient may be weird...
...
% TODO I wonder if we could "replace" the derivatives so that the network trains?

% TODO Maybe the problem is related to "sample rate"? The discrete bottleneck benefits from a higher sample rate of 4.




% TODO Does this need to be discussed? We didn't really use it. The sample rate trick is still interesting, though, as is the effects on training time. Actually, does it really take longer, since when we tried the default Balle factorized model with lambda=0.0067, that's also in 180+ epochs and counting... WHAT. And didn't we try discrete_entropy_bottleneck with same lambda, or was it lower?
...
% TODO(figure): Describe
% TODO(figure): RD curves / table
% TODO(figure): Training curves
...
% TODO Shouldn't all this be in "future work"?
...
% TODO add another stride layer; and fix other mentions of "four-layer" to "five-layer".
...
%     Reshape((C, -1)),
%     conv1d(C, max(N, C // 2), kernel_size=5, groups=groups),
%     Interleave(groups),
...
% observed it becomes WORSE if unfreeze ...  TODO hypotheses...?
...
% The parameters only increased by 0.1M for low rate...
%
% % 3.0M → 3.1M  (low rate)
% % 7.0M → 7.1M  (high rate)
%
% 3.00M → 3.05M   (low rate)
% 7.00M → 7.10M   (high rate)
%
% Actual configurations used table:
% ...
...
\begin{table}[htbp]
  ...
  \begin{tabular}[]{cccccc}
    ...
    % Todo swap columns 4 and 5
    & \thead{Potential \\ BD-rate}
    & \thead{Our savings \\ (bpp)}
    & \thead{Our \\ BD-rate} \\
    % & & $\propto D_{\mathrm{KL}}(\boldvar{p} \parallel \boldvar{p_{\textnormal{default}}})$ & & $\propto D_{\mathrm{KL}}(\boldvar{p} \parallel \boldvar{\hat{p}})$ & \\
    \midrule
    % Q  & origbpp&potsave& potBD&our sav& ourBD \\
    1    &  0.123 & 0.011 & -8.9 & 0.010 & -8.3 \\
    2    &  0.189 & 0.016 & -8.5 & 0.014 & -7.5 \\
    3    &  0.288 & 0.023 & -8.0 & 0.021 & -7.2 \\
    4    &  0.440 & 0.034 & -7.7 & 0.031 & -6.9 \\
    5    &  0.648 & 0.050 & -7.7 & 0.045 & -7.0 \\
    6    &  0.967 & 0.079 & -8.2 & 0.069 & -7.1 \\
    7    &  1.351 & 0.111 & -8.2 & 0.098 & -7.2 \\
    8    &  1.833 & 0.148 & -8.1 & 0.129 & -7.0 \\
    mean &        &       & -8.1 &       & -7.3 \\
    \bottomrule
  \end{tabular}
\end{table}
...
% TODO Move formula to earlier section, then reference here?
...
% TODO Mention the following tables within the main body text.

\begin{table}[htbp]
  \centering
  \caption{%
    Comparison of rates for various models.  % equipped with probability distribution compression.
    Ratio is the fraction of the total rate occupied original unmodified model.
    % Might be more natural to report in "absolute" percentage of total bitstream, rather than percentage of the given latent bitstream? (i.e., multiply)
    % TODO Maybe Potential, Actual are better names than Gap, Gain?
  ...
  \begin{tabular}[]{ccccc}
    ...
    \\
    \midrule
      % NOTE: Their measured gap may differ because they may not have evaluated at the same lambda values / rate points. (TODO footnote?) Or maybe they only did the Q=1 model...!!! See if they say in the paper. Show our results for that too, for comparison. Maybe two separate tables...
    bmshj2018-factorized~\cite{balle2018variational} + \cite{balcilar2022amortizationgap} (Q=1)
      % &   100 & -9.5  & -6.79 & -6.79 \\
      &   100 & -9.45 & -6.79 & -6.79 \\  % TODO verify 9.45
...
\begin{table}[htbp]
  ...
  \begin{tabular}[]{ccccc}
    \toprule
    % NOTE: We can make the table more horizontally compact by swapping top to contain "Params, MACs/pixel".
    ...
    % TODO verify computations with strides... especially for ConvTranspose...
    % TODO And add more rows...
    \bottomrule
  \end{tabular}
\end{table}
...
% TODO(table): Compare against prior works (BD PSNR, Rate)
% TODO(table): Parameter counts, MACs
% TODO(table): Cost of distribution transmission (e.g. 0.0002 bpp or ___ bits or ___% of bitstream, for Kodak)?

% TODO format closer to table from ~\cite{balcilar2022amortizationgap}?
% e.g., "gap %" and EB's "ratio %" of total rate...?
% Only need FACTORIZED column: ratio, gap, gain.
% Also, include mbt2018-mean's gain...?
% and ~\cite{balcilar2022amortizationgap}'s results too for factorized and mbt...

% TODO Mention how unfreezing g_a, g_s, and entropy_bottleneck results in poorer performance than keeping them frozen.



% TODO(experiments):
%
% 5 layer network for even better results (note, should be config, so we can switch)
%
% PDF+RDOQ
%
...
% We showcased a method with benefits:
% ......
% Led to .... performance
...
% My results:
%
% - Latent optimization via RDOQ
% - Alternative formulation: discrete entropy bottleneck, "discrete indexing loss", "histogram" methods
% - Entropy model optimization via adaptive probability distribution modeling
%
%
%
...
% Future work:
% - Entropy bottleneck (Perhaps I can leave improvements such as figuring out why training a "pdf-adaptive" image compression model with unfrozen g_a, g_s, and entropy_botteneck results in poorer performance than keeping them frozen.)

% the jointly optimized to-end
...
% Exploring methods for modeling entropy bottleneck
%
% See OneNote for methods including "DiscreteEntropyBottleneck",
% probability distribution histogram methods (KDE for triangle/gaussian), hard histogram, CDFHistogram.
...
% TODO(define): bpp, BD, RD, KL, etc. Can be defined on abbreviations page? ehhh I guess.
% TODO(math): Consistency: R_y, or R_{\boldvar{\tilde{y}}}? Same for R_q.
